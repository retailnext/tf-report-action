...
2. If the integration snapshot tests now fail, update the Vitest snapshots:
   ```bash
   npm run test -- --update-snapshots
   ```

3. Review the snapshot diffs carefully — they show the exact markdown output that
   changed. Confirm every change is intentional.

4. Commit both the updated JSON files and any updated snapshot files together in one commit.
