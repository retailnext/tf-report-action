...
describe("deriveModuleAddress", () => {
  ...
  it("extracts single-level module address", () => {
    expect(
      deriveModuleAddress("module.child.aws_instance.web", "aws_instance"),
    ).toBe("module.child");
  });
...
describe("deriveInstanceName", () => {
  ...
  it("returns name with numeric index", () => {
    expect(deriveInstanceName("null_resource.item[0]", "null_resource")).toBe(
      "item[0]",
    );
  });
  ...
  it("returns name with string key", () => {
    expect(
      deriveInstanceName('aws_instance.db["primary"]', "aws_instance"),
    ).toBe('db["primary"]');
  });

  it("strips module prefix from nested address", () => {
    expect(
      deriveInstanceName("module.child.aws_instance.web[0]", "aws_instance"),
    ).toBe("web[0]");
  });
  ...
  it("returns full address when type not found", () => {
    expect(deriveInstanceName("something_else.foo", "aws_instance")).toBe(
      "something_else.foo",
    );
  });
